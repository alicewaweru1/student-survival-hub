const WelcomeCard = ({ profile }) => {
  return (
    <div className="rounded-[30px] bg-[linear-gradient(135deg,#6f8b6b_0%,#4a2e25_100%)] px-6 py-8 text-white shadow-[0_25px_60px_-30px_rgba(74,46,37,0.5)] sm:px-8">
      ...
      <h1 className="mt-3 text-3xl font-bold sm:text-4xl">{profile?.name || "Amina"}, you&apos;re crushing this week.</h1>
      ...
      <div className="mt-5 flex flex-wrap gap-3 text-sm">
        <span className="rounded-full bg-white/15 px-4 py-2">{profile?.school || "Green Valley College"}</span>
        <span className="rounded-full bg-white/15 px-4 py-2">{profile?.major || "Business & Tech"}</span>
